 ...
 * Flota model.
 */
class FlotaModelRutaForm extends JModelForm
{
	...
	public function &getData($id = null)
	{
		if ($this->_item === null)
		{
			...
			if ($table !== false && $table->load($id))
			{
				...
				if (!$canEdit && $user->authorise('core.edit.own', 'com_flota'))
				{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_flota.ruta', 'rutaform', array(
			'control'   => 'jform',
			'load_data' => $loadData
		));
		if (empty($form))
	...
	{
		$data = JFactory::getApplication()->getUserState('com_flota.edit.ruta.data', array());
		if (empty($data))
		{
			$data = $this->getData();
		}
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ruta.id');
		...
		if ($id)
		{
			...
			$authorised = $user->authorise('core.edit', 'com_flota') || $authorised = $user->authorise('core.edit.own', 'com_flota');
			if ($user->authorise('core.edit.state', 'com_flota') !== true && $state == 1)
	...
	{
		$id = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('ruta.id');
		if (JFactory::getUser()->authorise('core.delete', 'com_flota') !== true)
		{
